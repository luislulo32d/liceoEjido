...
for($i = 0; $i < count($consulta);$i++) {
    ...
    if ($consulta[$i]['rol'] == 3) {
        $consulta[$i]['acciones'] = '
        <button class="btn btn-primary btn-sm" title="Editar" onclick="editarUsuario('.$consulta[$i]['usuario_id'].')" disabled>Editar</button>
        <button class="btn btn-danger btn-sm" title="Eliminar" onclick="eliminarUsuario('.$consulta[$i]['usuario_id'].')" disabled>Eliminar</button>
                                ';
    }else {
        $consulta[$i]['acciones'] = '
